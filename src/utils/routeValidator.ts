...
export class RotaValidator {
  routeValidate(
    actualRoute: ITracedRoute,
    expectedRoute: IExpectedRoute[] | unknown[],
  ): boolean {
  ...
  calculateDistanceBetweenTwoPoints(actualRoute, expectedRoute): number {
    ...
    const latitudeDifference =
      ((expectedRoute.latitude - actualRoute.latitude) * Math.PI) / 180;
    const longitudeDifference =
      ((expectedRoute.longitude - actualRoute.longitude) * Math.PI) / 180;
